...
describe('Sequence must match respective path', () => {
  test('must match the path', () => {
    ...
    expect(edgecaseSequence[3].getNextStep({})).toBe(USER_ROLE);

    expect(edgecaseSequence[4].url).toBe(USER_ROLE);
    ...
    expect(edgecaseSequence[8].url).toBe(MANUAL_ADDRESS);
    expect(edgecaseSequence[8].getNextStep({})).toBe(CONTACT_PREFERENCES);

    expect(edgecaseSequence[9].url).toBe(CONTACT_PREFERENCES);
