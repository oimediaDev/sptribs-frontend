...
describe('Sequence must match respective path', () => {
  test('must match the path', () => {
    ...
    expect(edgecaseSequence[4].getNextStep({})).toBe(USER_ROLE);

    expect(edgecaseSequence[5].url).toBe(USER_ROLE);
    expect(edgecaseSequence[5].getNextStep({})).toBe(DATE_OF_BIRTH);

    expect(edgecaseSequence[6].url).toBe(DATE_OF_BIRTH);
    expect(edgecaseSequence[6].getNextStep({})).toBe(FIND_ADDRESS);

    expect(edgecaseSequence[7].url).toBe(FIND_ADDRESS);
    expect(edgecaseSequence[7].getNextStep({})).toBe(SELECT_ADDRESS);

    expect(edgecaseSequence[8].url).toBe(SELECT_ADDRESS);
    expect(edgecaseSequence[8].getNextStep({})).toBe(CONTACT_PREFERENCES);

    expect(edgecaseSequence[9].url).toBe(MANUAL_ADDRESS);
    expect(edgecaseSequence[9].getNextStep({})).toBe(CONTACT_PREFERENCES);

    expect(edgecaseSequence[10].url).toBe(CONTACT_PREFERENCES);
    expect(edgecaseSequence[10].getNextStep({})).toBe(UPLOAD_YOUR_DOCUMENTS);
